...
//DENSITY
TEXTURE2D(_GrassDensityMask); SAMPLER(sampler_GrassDensityMask); float4 _GrassDensityMask_ST;
TEXTURE2D(_TransparencyNoiseTexture); SAMPLER(sampler_TransparencyNoiseTexture); float4 _TransparencyNoiseTexture_ST;
...
VertexOutput Vertex(Attributes input) {
    ...
    output.normalWS = float3(0,1,0);//input.normalOS; //normalInput.normalWS;
...
half4 Fragment(GeometryOutput input) : SV_Target {
	...
	float aldeboNoise = SAMPLE_TEXTURE2D(_GrassDensityMask, sampler_GrassDensityMask, TRANSFORM_TEX(uv, _GrassDensityMask)).b;
	...
	//aldeboNoise = 1 - (1 - aldeboNoise) * _SmoothDepthScale;
	...
	//TRYING TO SETUP COLOR
	float detailAlbedo = SAMPLE_TEXTURE2D(_TransparencyNoiseTexture, sampler_TransparencyNoiseTexture, TRANSFORM_TEX(uv, _TransparencyNoiseTexture)).r;
    float3 albedo = lerp(_BaseColor, _TopColor, height).rgba;
    ...
    return UniversalFragmentBlinnPhong(lightingInput, albedo * (detailAlbedo + 0.6), 1, 0, 0, 0);
#endif
}
